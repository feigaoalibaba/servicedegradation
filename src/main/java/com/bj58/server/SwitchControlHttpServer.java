...
/**
 * 内置的监听服务 可以通过其它方式替换 如mq 或 zookeeper
 */
...
class HttpFileHandler implements HttpRequestHandler {
	...
	public void handle(final HttpRequest request, final HttpResponse response,
		...
		if (methodstatus != null && "open".equalsIgnoreCase(methodstatus)) {
			ServiceStatusManager.getInstance().openService(classmethod);
			boolean ret = ObjectProductionFactory.addJsonRet(classmethod,
					jsonResult, classType);
		} else if (methodstatus != null
				&& "close".equalsIgnoreCase(methodstatus)) {
			ServiceStatusManager.getInstance().closeService(classmethod);
			ObjectProductionFactory.remove(classmethod);
		}
